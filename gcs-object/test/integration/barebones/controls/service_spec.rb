...
set_project = `gcloud auth activate-service-account --key-file /root/static_build/credentials.json`
...
control "get-gcs-object" do
  ...
  instances_list = JSON.parse(`gcloud compute instances list --filter="name:( uw1-tst-test-app-get-gcs-object-server-type-0001-0-* )  status:( RUNNING )" --format json`)
  # puts instance_list
