...
@test "Test that project has the shared vpc associated (host project)" {
  ...
  run gcloud compute shared-vpc get-host-project $PROJECT_ID --format=flattened[no-pad]
  [ "$status" -eq 0 ]
  [[ "${lines[1]}" = "name: $SHARED_VPC" ]]
}
...
@test "Test service account has compute.instanceAdmin.v1 role on project" {

  export PROJECT_ID="$(terraform output project_info_example)"
  export GROUP_EMAIL="$(terraform output group_email_example)"

  run gcloud projects get-iam-policy $PROJECT_ID --format=list[compact] --filter="bindings.role=roles/compute.instanceAdmin.v1 AND bindings.members=serviceAccount:project-service-account@$PROJECT_ID.iam.gserviceaccount.com"
  [ "$status" -eq 0 ]
  [[ "$output" = *"u'role': u'roles/compute.instanceAdmin.v1', u'members': [u'serviceAccount:project-service-account@$PROJECT_ID.iam.gserviceaccount.com']"* ]]
}
...
@test "Test Gsuite group $GROUP_EMAIL has role:$GROUP_ROLE on project" {
  ...
  run gcloud projects get-iam-policy $PROJECT_ID --format=list[compact] --filter="bindings.role=$GROUP_ROLE AND bindings.members=group:$GROUP_EMAIL"
  [ "$status" -eq 0 ]
