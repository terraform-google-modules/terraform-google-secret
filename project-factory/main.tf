...
locals {
  ...
  project_id             = "${join("", google_project.project.*.project_id)}"
  project_number         = "${join("", google_project.project.*.number)}"
  ...
  s_account_id           = "${join("", google_service_account.default_service_account.*.account_id)}"
  s_account_email        = "${join("", google_service_account.default_service_account.*.email)}"
  s_account_name         = "${join("", google_service_account.default_service_account.*.name)}"
  s_account_display_name = "${join("", google_service_account.default_service_account.*.display_name)}"
  s_account_unique_id    = "${join("", google_service_account.default_service_account.*.unique_id)}"
  s_account_fmt          = "${format("serviceAccount:%s", local.s_account_email)}"
...
resource "google_project_service" "project_services" {
  count = "${local.enabled ? length(var.activate_apis) : 0}"
...
resource "google_compute_shared_vpc_service_project" "shared_vpc_attachment" {
  count = "${(local.enabled && var.shared_vpc != "") ? 1 : 0}"
...
resource "null_resource" "delete_default_compute_service_account" {
  ...
  provisioner "local-exec" {
    command = "python3 ${path.module}/scripts/delete_default_compute_service_account.py --project_id ${local.project_id} --path ${var.credentials_path}"
  }
...
resource "google_project_iam_binding" "default_service_account_policy" {
  count   = "${(local.enabled && var.sa_role != "") ? 1 : 0}"
  project = "${local.project_id}"
  role    = "${var.sa_role}"
...
resource "google_project_iam_member" "gsuite_group_role" {
  count = "${(local.enabled && local.gsuite_group) ? 1 : 0}"
...
resource "google_project_iam_binding" "controlling_group_vpc_role" {
  count = "${(local.enabled && var.shared_vpc != "" && (length(compact(var.shared_vpc_subnets)) == 0)) ? 1 : 0}"
...
resource "google_compute_subnetwork_iam_member" "service_account_role_to_vpc_subnets" {
  count = "${(local.enabled && var.shared_vpc != "" && (length(compact(var.shared_vpc_subnets)) > 0)) ? length(var.shared_vpc_subnets) : 0 }"
...
resource "google_compute_subnetwork_iam_member" "group_role_to_vpc_subnets" {
  count = "${(local.enabled && var.shared_vpc != "" && (length(compact(var.shared_vpc_subnets)) > 0)) && local.gsuite_group ? length(var.shared_vpc_subnets) : 0 }"
...
resource "google_project_usage_export_bucket" "usage_report_export" {
  count = "${(local.enabled && (var.usage_bucket_name != "")) ? 1 : 0}"
...
resource "google_storage_bucket" "project_bucket" {
  count = "${(local.enabled && local.create_bucket) ? 1 : 0}"
...
resource "google_storage_bucket_iam_member" "group_storage_admin_on_project_bucket" {
  count = "${(local.enabled && local.create_bucket && local.gsuite_group) ? 1 : 0}"
