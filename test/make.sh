...
# to exit on error
function check_bash() {
find . -name "*.sh" -not -path "*/.terraform/*" | while IFS= read -d '' -r file;
do
  if [[ "$file" != *"bash -e"* ]];
  then
...
function docker() {
  ...
  find . -name "Dockerfile" -not -path "*/.terraform/*" -exec hadolint {} \;
}
...
function golang() {
  ...
  find . -name "*.go" -not -path "*/.terraform/*" -exec go fmt {} \;
  find . -name "*.go" -not -path "*/.terraform/*" -exec go vet {} \;
}
...
function check_python() {
  echo "Running flake8"
  find . -name "*.py" -not -path "*/.terraform/*" -exec flake8 {} \;
}
...
function check_shell() {
  ...
  find . -name "*.sh" -not -path "*/.terraform/*" -exec shellcheck -x {} \;
}
